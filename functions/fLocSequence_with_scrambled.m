classdef fLocSequence_with_scrambled
    ...
    properties (Constant)
        stim_conds = {'Bodies' 'Characters' 'Faces' 'Objects' 'Places' 'Scrambled'};
        stim_per_block = 12;   % number of stimuli in a block
        stim_duty_cycle = 0.5; % duration of stimulus duty cycle (s)
    end
    
    properties (Constant, Hidden)
        stim_set1 = {'body' 'word' 'adult' 'car' 'house' 'scrambled1'};
        stim_set2 = {'limb' 'number' 'child' 'instrument' 'corridor' 'scrambled2'};
        stim_per_set = 144;
    ...
    methods
        ...
        % class constructor
        function seq = fLocSequence_with_scrambled(stim_set, num_runs, task_num)
            if nargin < 1
                seq.stim_set = 3;
            else
